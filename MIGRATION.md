...
moonsic v2 采用多包架构，将原来的单根包拆分为 25 个独立功能子包。根包 `hyl-star/moonsic` 不再导出任何 API，所有原有符号已迁移到 `packages/` 下的对应子包。

## 核心变更

| 变更类型 | v1 | v2 |
|---------|----|----|
| 包结构 | 单一根包 `hyl-star/moonsic` | 25 个子包在 `packages/` 下 |
...
{
  "import": [
    { "path": "hyl-star/moonsic/packages/pitch" },
    { "path": "hyl-star/moonsic/packages/time" },
    { "path": "hyl-star/moonsic/packages/score" },
    { "path": "hyl-star/moonsic/packages/midi" },
    { "path": "hyl-star/moonsic/packages/musicxml" }
  ]
}
```
...
根包 `hyl-star/moonsic` 的 `moon.pkg` 目前为空。所有旧文件已从根包移除。v1 API 不再可用。
